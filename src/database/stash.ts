...
export class StashBucket<T extends BaseItem, Meta = any, Config = any> {
  ...
  // 用于存储全局信息
  get metaStorage() {
    return this.metaFactory(StashBucket.META_NAME) as ReturnType<StorageFactory<Meta>>;
  }
  // 用于存储数组增量信息
  get stashStorage() {
  ...
  // 用于保存本地额外信息
  get configStorage() {
    return this.metaFactory(StashBucket.CONFIG_NAME) as ReturnType<StorageFactory<Config>>;
  }
  ...
  async batch(actions: Action<T>[], overlap = false) {
    ...
    const densed = denseStashes([...fullActions, ...(overlap ? [] : prevActions)]);
    if (overlap) {
      const now = Date.now();
...
function denseStashes<T extends BaseItem>(stashes: FullAction<T>[]) {
  ...
  return stashes.filter((ac) => {
    const targetId = ac.type === "meta" ? "_meta" : ac.type === "delete" ? ac.value : ac.value.id;
    if (ids.has(targetId)) {
      return false;
    }
    ids.add(targetId);
